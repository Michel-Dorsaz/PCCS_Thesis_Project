...
{
    /// <summary>
    /// This UserControl is used to display and manage the recipes and their category 
    /// inside a treeView.
    /// </summary>
    public partial class RecipesTreeviewControl : UserControl
    {
        ...
        public RecipesTreeviewControl(FormEdit formEdit)
        {
            InitializeComponent();
            ...
            Selection = new RecipeSelectionHandler(this, formEdit.GetRightPanel(), formEdit.GetInfoLabel());
        ...
        /// load the recipes and recipesCategories into the TreeView.
        /// </summary>
        private void LoadData()
        {
            Log.Information("RecipesTreeViewControl - LoadData()");
        ...
        private void SetUpEventHandlers(FormEdit formEdit)
        {
            ...
            buttonNewRecipe.MouseDown += new MouseEventHandler(ButtonRecipe_MouseDown);
            ...
            buttonNewRecipe.GiveFeedback += (sender, e) => DragControl(buttonNewRecipe, e, true);
        ...
        {
            Log.Information("RecipesTreeViewControl - FormEdit closing");
        ...
        /// <summary>
        /// Initiate the drag of a new Recipe.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ButtonRecipe_MouseDown(object? sender, MouseEventArgs e)
        {
            Recipe newRecipe = new Recipe();

            buttonNewRecipe.DoDragDrop(newRecipe, DragDropEffects.Copy);
        }
        ...
        /// <summary>
        /// When a drag enter the TreeView; the drag is allowed only for recipes and their categories.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void TreeView_DragEnter(object? sender, DragEventArgs e)
        {
            ...
            // The drag inside the treeView is allow only for recipes and their categories
        ...
        private void TreeView_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            InfoLabel.Text = "Updating...";
            Log.Information("RecipeTreeViewControl - Drop at {Target}", targetNode);
        ...
        private void ButtonDelete_DragEnter(object? sender, DragEventArgs e)
        {
            ...
            // Check, if the instance is not null (meaning e.Data was of type Recipe or recipe category)
        ...
        private void ButtonDelete_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            /*
             * Only an EXISTING recipe or a category can be deleted.
             * Thus, we create both and check whether they exist and correspond to 
             * appropried category
             */

            // Create both instances
            Recipe? recipe = (Recipe) e.Data.GetData(typeof(Recipe));
            RecipeCategory? category = (RecipeCategory)e.Data.GetData(typeof(RecipeCategory));

            InfoLabel.Text = "Deleting...";
            Log.Information("RecipesTreeViewControl - Drop at delete button");

            // Check, if the instance is not null (meaning e.Data was that type)
            if (recipe != null)
                RecipesManager.Delete(recipe);
            else if (category != null)
                RecipesManager.Delete(category);
        ...
        {
            TreeElement recipe = (TreeElement)e.Data.GetData(typeof(Recipe));
            TreeElement category = (TreeElement)e.Data.GetData(typeof(RecipeCategory));
